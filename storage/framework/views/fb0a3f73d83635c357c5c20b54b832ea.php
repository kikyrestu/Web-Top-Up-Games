<?php $__env->startSection('title', 'Edit Game / Layanan'); ?>
<?php $__env->startSection('header', 'Edit: ' . $category->name); ?>

<?php $__env->startSection('content'); ?>
<div class="bg-gray-800 rounded-lg shadow-lg border border-gray-700 p-6 max-w-2xl" x-data="{ categoryType: '<?php echo e(old('type', $category->type)); ?>' }">
    <form action="<?php echo e(route('admin.categories.update', $category->id)); ?>" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="name" id="name" value="<?php echo e(old('name', $category->name)); ?>" class="w-full px-3 py-2 bg-gray-700 border-gray-600 text-white rounded focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500" required>
...
unset($__errorArgs, $__bag); ?>
        ...
        <div class="mb-4">
            ...
            <select name="type" id="type" x-model="categoryType" class="w-full px-3 py-2 bg-gray-700 border-gray-600 text-white rounded focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500" required>
                <optgroup label="🎮 Game">
                    <option value="game" <?php echo e(old('type', $category->type) == 'game' ? 'selected' : ''); ?>>Game (Umum)</option>
                    <option value="seluler" <?php echo e(old('type', $category->type) == 'seluler' ? 'selected' : ''); ?>>Game Seluler (Mobile)</option>
                    <option value="pc" <?php echo e(old('type', $category->type) == 'pc' ? 'selected' : ''); ?>>Game PC</option>
                    <option value="voucher" <?php echo e(old('type', $category->type) == 'voucher' ? 'selected' : ''); ?>>Voucher</option>
                </optgroup>
                <optgroup label="💡 PPOB / Tagihan">
                    <option value="pulsa" <?php echo e(old('type', $category->type) == 'pulsa' ? 'selected' : ''); ?>>Pulsa</option>
                    <option value="paket_data" <?php echo e(old('type', $category->type) == 'paket_data' ? 'selected' : ''); ?>>Paket Data</option>
                    <option value="pln" <?php echo e(old('type', $category->type) == 'pln' ? 'selected' : ''); ?>>PLN / Token Listrik</option>
                    <option value="pdam" <?php echo e(old('type', $category->type) == 'pdam' ? 'selected' : ''); ?>>PDAM</option>
                    <option value="bpjs" <?php echo e(old('type', $category->type) == 'bpjs' ? 'selected' : ''); ?>>BPJS</option>
                    <option value="internet" <?php echo e(old('type', $category->type) == 'internet' ? 'selected' : ''); ?>>Internet / TV Kabel</option>
                    <option value="emoney" <?php echo e(old('type', $category->type) == 'emoney' ? 'selected' : ''); ?>>E-Money / E-Wallet</option>
                    <option value="ppob" <?php echo e(old('type', $category->type) == 'ppob' ? 'selected' : ''); ?>>PPOB Lainnya</option>
                </optgroup>
            </select>
...
unset($__errorArgs, $__bag); ?>
        ...
        <div class="mb-4" x-show="['game','seluler','pc','voucher'].includes(categoryType)" x-transition>
            ...
            <input type="text" name="publisher" id="publisher" value="<?php echo e(old('publisher', $category->publisher)); ?>" placeholder="contoh: Moonton" class="w-full px-3 py-2 bg-gray-700 border-gray-600 text-white rounded focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">
...
unset($__errorArgs, $__bag); ?>
        ...
        <div class="mb-4">
            ...
            <?php if($category->thumbnail): ?>
                <div class="mb-3">
                    <img src="<?php echo e(Storage::url($category->thumbnail)); ?>" alt="Thumbnail Saat Ini" class="h-24 w-auto object-cover rounded shadow">
...
unset($__errorArgs, $__bag); ?>
        ...
        <div class="mb-4">
            ...
            <textarea name="description" id="description" rows="3" class="w-full px-3 py-2 bg-gray-700 border-gray-600 text-white rounded focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"><?php echo e(old('description', $category->description)); ?></textarea>
        </div>

        <div class="mb-4 flex space-x-4">
            <div class="w-1/2">
                <label for="sort_order" class="block text-gray-300 text-sm font-bold mb-2">Urutan Tampil</label>   
                <input type="number" name="sort_order" id="sort_order" value="<?php echo e(old('sort_order', $category->sort_order)); ?>" class="w-full px-3 py-2 bg-gray-700 border-gray-600 text-white rounded focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">
            </div>
            <div class="w-1/2 flex items-end pb-2">
                <label class="flex items-center space-x-2 cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" class="form-checkbox h-5 w-5 text-blue-600" <?php echo e(old('is_active', $category->is_active) ? 'checked' : ''); ?>>
        ...
        <div class="mb-4 flex space-x-4">
            <div class="w-1/2 flex items-end pb-2">
                <label class="flex items-center space-x-2 cursor-pointer">
                    <input type="checkbox" name="is_popular" value="1" class="form-checkbox h-5 w-5 text-yellow-500" <?php echo e(old('is_popular', $category->is_popular) ? 'checked' : ''); ?>>
                    <span class="text-sm font-bold text-gray-300">Tampilkan sebagai Populer</span>
                </label>
            </div>
            <div class="w-1/2 flex items-end pb-2">
                <label class="flex items-center space-x-2 cursor-pointer">
                    <input type="checkbox" name="is_new" value="1" class="form-checkbox h-5 w-5 text-green-500" <?php echo e(old('is_new', $category->is_new) ? 'checked' : ''); ?>>
        ...
        <div class="mb-4 border border-gray-600 rounded-lg p-4 bg-gray-900/50" x-data="formTemplateBuilder(<?php echo e(json_encode($category->input_fields ?? [])); ?>)">
        ...
        <div class="flex justify-end space-x-3 mt-6">
            <a href="<?php echo e(route('admin.categories.index')); ?>" class="bg-gray-900/500 hover:bg-gray-600 text-white font-bold py-2 px-4 rounded transition">Batal</a>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\PROJECT\webppobdantopup\resources\views/admin/categories/edit.blade.php ENDPATH**/ ?>