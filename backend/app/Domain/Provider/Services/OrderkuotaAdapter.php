...
final class OrderkuotaAdapter implements ProviderAdapterInterface
{
    /**
    ...
    {
        $baseUrl = rtrim((string) config('services.orderkuota.base_url'), '/');
        $username = (string) config('services.orderkuota.username');
        $apiKey = (string) config('services.orderkuota.api_key');
    ...
    public function createOrder(array $payload): array
    {
        ...
        if ($baseUrl === '' || $username === '' || $apiKey === '') {
            return [
                ...
                'is_retryable' => false,
                'provider_ref' => null,
                'raw' => ['error' => 'missing_orderkuota_config'],
            ];
        }
