...
<div class="container mx-auto px-4 mt-20 md:mt-24 pb-20" x-data="checkoutPage()">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div class="lg:col-span-8 flex flex-col gap-5 md:gap-6">
            ...
            <div class="bg-[#1c1c1c] rounded-xl border border-[#2d2d2d] overflow-hidden">
                ...
                <div class="p-4 md:p-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <input type="text" x-model="target" placeholder="Masukkan ID" class="w-full bg-[#121212] border border-[#333] px-4 py-3 rounded-lg focus:outline-none focus:border-[#f97316] focus:ring-1 focus:ring-[#f97316] transition text-white placeholder-gray-500 text-sm">
                        </div>
                        <div>
                            <input type="text" x-model="targetZone" placeholder="Masukkan ID Server (Jika Perlu)" class="w-full bg-[#121212] border border-[#333] px-4 py-3 rounded-lg focus:outline-none focus:border-[#f97316] focus:ring-1 focus:ring-[#f97316] transition text-white placeholder-gray-500 text-sm">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-[#1c1c1c] rounded-xl border border-[#2d2d2d] overflow-hidden">
                ...
                <div class="p-4 md:p-6">
                    ...
                    @else
                        <!-- Group Header Mockup 1: Flash Sale -->
                        <div class="border border-[#f97316] rounded-xl p-4 md:p-5 relative mb-6">
                            <h3 class="absolute -top-4 left-4 bg-[#1c1c1c] px-3 flex items-center gap-2 text-[#f97316] font-black text-lg italic">
                                <i class="fas fa-bolt"></i> FLASH SALE
                            </h3>
                            <div class="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4 mt-2">
                                @foreach($products->take(2) as $product)
                                <div class="border-2 border-[#333] hover:border-[#f97316] bg-[#222] rounded-xl p-3 md:p-4 cursor-pointer relative overflow-hidden group transition-all"
                                     :class="{'border-[#f97316] bg-[#f97316]/5': selectedProduct === {{ $product->id }}}"
                                     @click="selectProduct({{ $product->id }}, '{{ $product->name }}', {{ $product->price_sell }})">
                                    ...
                                    <div class="text-[10px] md:text-xs text-gray-500 line-through mt-0.5">
                                        Rp {{ number_format($product->price_sell + 5000, 0, ',', '.') }}
                                    </div>

                                    <div class="mt-3 flex items-center gap-2">
                                        <span class="bg-[#f97316] text-white text-[9px] md:text-[10px] font-bold px-2 py-0.5 rounded">Disc 5%</span>
                                    </div>
                        ...
                        <!-- Group Header Mockup 2: Membership / Reguler -->
                        <div class="mb-4 text-[#f59e0b] font-bold text-sm italic py-2 flex items-center gap-2">
                            <i class="fas fa-star"></i> REGULER ITEM
                        </div>
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4">
                            ...
                            <div class="border border-[#333] hover:border-[#f97316] bg-[#222] rounded-xl p-3 md:p-4 cursor-pointer relative overflow-hidden group transition-all"
                                ...
                                <div class="text-[10px] md:text-xs text-gray-500 line-through mt-0.5">
                                    Rp {{ number_format($product->price_sell + 2000, 0, ',', '.') }}
                                </div>
            ...
            <div class="bg-[#1c1c1c] rounded-xl border border-[#2d2d2d] overflow-hidden">
                <div class="bg-[#151515] px-4 py-3 md:p-4 border-b border-[#2d2d2d] flex items-center gap-3">
                    <span class="bg-[#f97316] text-white w-7 h-7 md:w-8 md:h-8 rounded-full flex items-center justify-center font-bold text-sm shadow-md">6</span>
                    <h2 class="text-base md:text-lg font-bold text-white">Kode Promo</h2>
                </div>
                <div class="p-4 md:p-6">
                    <div class="flex gap-2">
                        <input type="text" x-model="promo" placeholder="Ketik Kode Promo Kamu" class="w-full bg-[#121212] border border-[#333] px-4 py-3 rounded-lg focus:outline-none focus:border-[#f97316] focus:ring-1 focus:ring-[#f97316] transition text-white placeholder-gray-500 text-sm">
                        <button type="button" class="bg-[#f97316] hover:bg-[#ea580c] text-white px-6 py-3 rounded-lg font-bold text-sm transition-colors shadow-[0_0_15px_rgba(249,115,22,0.3)] whitespace-nowrap">Gunakan</button>
                    </div>
                    <button class="mt-4 flex items-center gap-2 text-sm text-[#f97316] font-semibold bg-[#f97316]/10 px-4 py-2 rounded-lg border border-[#f97316]/20 hover:bg-[#f97316]/20 transition"><i class="fas fa-ticket-alt"></i> Pakai Promo Yang Tersedia</button>
                </div>
            </div>
            ...
            <div class="mt-2" x-data="{ activeFaq: null }">
                ...
                <div class="mt-6 bg-[#f97316]/10 border border-[#f97316] rounded-xl p-4 md:p-6 pb-2">
                    <p class="text-[11px] text-gray-300 leading-relaxed mb-4">
                        <strong class="text-[#f97316]">Free Fire</strong> adalah game battle royale mobile yang dikembangkan oleh Garena. 
                        Dalam game ini, pemain akan bertarung dalam sebuah arena yang luas dengan 50 pemain lain, bertujuan untuk menjadi yang terakhir bertahan hidup. 
                        Pemain dapat memilih lokasi pendaratan, mencari senjata, perlengkapan, dan bertempur melawan pemain lain hingga hanya satu pemain atau tim yang tersisa.
                    </p>
                    <p class="text-center pb-2">
                        <a href="#" class="text-[#f97316] text-xs font-semibold hover:underline">Baca Selengkapnya</a>
                    </p>
                </div>
            </div>
        ...
        <div class="lg:col-span-4 flex flex-col gap-5 md:gap-6">
            ...
            <!-- Rating Box -->
            <div class="bg-[#1c1c1c] rounded-xl border border-[#2d2d2d] p-5">
                <p class="text-sm text-gray-400 font-semibold mb-2">Ulasan dan rating</p>
                <div class="flex items-end gap-3 mb-2">
                    <span class="text-4xl font-black text-white leading-none">4.99</span>
                    <div class="flex text-[#f59e0b] text-base mb-1">
                ...
                <p class="text-[11px] text-gray-500">Berdasarkan total 17,046 rating</p>
            </div>
