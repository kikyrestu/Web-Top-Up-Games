                                                ...
                                                <button type="button" @click="selectedProduct = product.id; showProductModal = false"
                                                    ...
                                                    <div class="flex items-center gap-3">
                                                        <div class="font-bold text-sm whitespace-nowrap"
                                                             :class="(activeType === 'tagihan' || activeType === 'pln' || product.product_type === 'postpaid') ? 'text-green-400' : ((product.price_sell ?? product.price) <= 0 ? 'text-cyan-400' : 'text-up-yellow')"
                                                             x-text="(activeType === 'tagihan' || activeType === 'pln' || product.product_type === 'postpaid') ? 'Cek Tagihan' : ((product.price_sell ?? product.price) <= 0 ? 'Input Nominal' : 'Rp ' + new Intl.NumberFormat('id-ID').format(product.price_sell ?? product.price))"></div>
