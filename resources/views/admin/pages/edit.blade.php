...
<div class="bg-dark-800/40 backdrop-blur-xl rounded-2xl border border-dark-600/50 shadow-2xl p-6 max-w-4xl mx-auto">
    <form action="{{ route('admin.pages.update', $page) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="title" value="{{ old('title', $page->title) }}" required class="w-full bg-dark-900 border border-dark-600 text-white text-sm rounded-xl p-3">
            @error('title')<p class="text-rose-500 text-xs mt-1">{{ $message }}</p>@enderror
        </div>

        <div class="mb-4">
            <label class="block text-gray-400 text-xs font-bold uppercase tracking-wider mb-2">Slug URL</label>
            <input type="text" name="slug" value="{{ old('slug', $page->slug) }}" required class="w-full bg-dark-900 border border-dark-600 text-white text-sm rounded-xl p-3 font-mono">
        ...
        <div class="mb-4">
            ...
            <textarea id="editor" name="content" rows="15" class="w-full bg-dark-900 border border-dark-600 text-white text-sm rounded-xl p-3 font-mono">{!! old('content', $page->content) !!}</textarea>
            @error('content')<p class="text-rose-500 text-xs mt-1">{{ $message }}</p>@enderror
        </div>

        <div class="mb-6">
            <label class="inline-flex items-center gap-2 cursor-pointer">
                <input type="checkbox" name="is_active" value="1" {{ old('is_active', $page->is_active) ? 'checked' : '' }} class="rounded border-dark-600 bg-dark-900 text-emerald-500 focus:ring-emerald-500/40 w-5 h-5">
