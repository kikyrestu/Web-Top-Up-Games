            ...
            <form action="{{ route('admin.banners.update', $banner->id) }}" method="POST" enctype="multipart/form-data" class="relative z-10 space-y-6">
                ...
                <div class="bg-dark-800/50 p-5 rounded-xl border border-dark-700">
                    ...
                    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
                        ...
                        <label class="relative cursor-pointer">
                            <input type="radio" name="media_type" value="image" x-model="mediaType" class="peer sr-only" {{ old('media_type', $banner->media_type) == 'image' ? 'checked' : '' }}>
                        ...
                        <label class="relative cursor-pointer">
                            <input type="radio" name="media_type" value="video" x-model="mediaType" class="peer sr-only" {{ old('media_type', $banner->media_type) == 'video' ? 'checked' : '' }}>
                            <div class="p-3 text-center rounded-xl bg-dark-900 border border-dark-600 peer-checked:border-brand-500 peer-checked:bg-brand-500/10 peer-checked:text-brand-400 text-gray-400 transition-all hover:bg-dark-800 hover:border-gray-600 shadow-sm disabled:opacity-50">
                        ...
                        <label class="relative cursor-pointer">
                            <input type="radio" name="media_type" value="embed" x-model="mediaType" class="peer sr-only" {{ old('media_type', $banner->media_type) == 'embed' ? 'checked' : '' }}>
                        ...
                        <label class="relative cursor-pointer">
                            <input type="radio" name="media_type" value="html" x-model="mediaType" class="peer sr-only" {{ old('media_type', $banner->media_type) == 'html' ? 'checked' : '' }}>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title', $banner->title) }}" class="w-full px-4 py-2.5 bg-dark-800 border border-dark-600 text-white rounded-xl focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition-all">
                ...
                <div class="mb-5">
                    ...
                    <select name="position" id="position" class="w-full px-4 py-2.5 bg-dark-800 border border-dark-600 text-white rounded-xl focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition-all appearance-none">
                        <option value="hero" {{ old('position', $banner->position) == 'hero' ? 'selected' : '' }}>🖼️ Hero Utama (Slider Banner Besar)</option>
                        <option value="ppob_promo" {{ old('position', $banner->position) == 'ppob_promo' ? 'selected' : '' }}>💳 Banner PPOB (Section Bayar & Tagihan)</option>
                        <option value="game_promo" {{ old('position', $banner->position) == 'game_promo' ? 'selected' : '' }}>🎮 Banner Game Promo</option>
                        <option value="popup" {{ old('position', $banner->position) == 'popup' ? 'selected' : '' }}>📢 Popup / Notifikasi</option>
...
<script>
    function bannerForm() {
        return {
            mediaType: '{{ old("media_type", $banner->media_type ?? "image") }}',
