    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <aside :class="sidebarOpen ? 'w-[270px] translate-x-0 border-r border-dark-700' : 'w-0 -translate-x-[270px] border-r-0'" class="flex-shrink-0 relative z-20 h-full flex flex-col overflow-hidden transition-all duration-300 ease-in-out bg-dark-800 shadow-2xl">
            ...
            <div class="h-20 flex items-center justify-between px-6 border-b border-dark-700 bg-dark-800 shrink-0">
                <div class="flex items-center gap-3 w-full" x-show="sidebarOpen">
                    @if(!empty($global_site_logo))
                        <img src="{{ asset('storage/' . $global_site_logo) }}" alt="{{ $global_site_name ?? 'Logo' }}" class="w-9 h-9 rounded-xl object-cover border border-dark-600">
                    @else
                    ...
                    @endif
                    <span class="text-xl font-bold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-white to-gray-300">{{ $global_site_name ?? 'PPOBKu' }}</span>
                </div>
            </div>
            ...
            <nav class="flex-1 overflow-y-auto p-4 space-y-1 custom-scrollbar">
                ...
                <a href="{{ route('admin.categories.index') }}" class="flex items-center space-x-3 {{ request()->routeIs('admin.categories.*') ? 'bg-brand-500/10 text-brand-400' : 'text-gray-400 hover:bg-dark-700/50 hover:text-white' }} px-4 py-3 rounded-xl transition duration-200 group">
                    <i class="fas fa-layer-group w-5 text-lg transition-transform group-hover:scale-110 {{ request()->routeIs('admin.categories.*') ? 'text-brand-400' : 'text-gray-500 group-hover:text-gray-300' }}"></i>
                    <span class="font-medium text-sm" x-show="sidebarOpen">Kategori</span>
                </a>
                
                <a href="{{ route('admin.products.index') }}" class="flex items-center space-x-3 {{ request()->routeIs('admin.products.*') ? 'bg-brand-500/10 text-brand-400' : 'text-gray-400 hover:bg-dark-700/50 hover:text-white' }} px-4 py-3 rounded-xl transition duration-200 group">
                    <i class="fas fa-box-open w-5 text-lg transition-transform group-hover:scale-110 {{ request()->routeIs('admin.products.*') ? 'text-brand-400' : 'text-gray-500 group-hover:text-gray-300' }}"></i>
                    <span class="font-medium text-sm" x-show="sidebarOpen">Produk</span>
                </a>

                <a href="{{ route('admin.product-sync.index') }}" class="flex items-center space-x-3 {{ request()->routeIs('admin.product-sync.*') ? 'bg-brand-500/10 text-brand-400' : 'text-gray-400 hover:bg-dark-700/50 hover:text-white' }} px-4 py-3 rounded-xl transition duration-200 group">
                    <i class="fas fa-sync-alt w-5 text-lg transition-transform group-hover:scale-110 {{ request()->routeIs('admin.product-sync.*') ? 'text-brand-400' : 'text-gray-500 group-hover:text-gray-300' }}"></i>
                    <span class="font-medium text-sm" x-show="sidebarOpen">Sync Produk</span>
                </a>

                <a href="{{ route('admin.scraped-products.index') }}" class="flex items-center space-x-3 {{ request()->routeIs('admin.scraped-products.*') ? 'bg-brand-500/10 text-brand-400' : 'text-gray-400 hover:bg-dark-700/50 hover:text-white' }} px-4 py-3 rounded-xl transition duration-200 group">
                    <i class="fas fa-database w-5 text-lg transition-transform group-hover:scale-110 {{ request()->routeIs('admin.scraped-products.*') ? 'text-brand-400' : 'text-gray-500 group-hover:text-gray-300' }}"></i>
                ...
                <a href="{{ route('admin.transactions.index') }}" class="flex items-center space-x-3 {{ request()->routeIs('admin.transactions.*') ? 'bg-brand-500/10 text-brand-400' : 'text-gray-400 hover:bg-dark-700/50 hover:text-white' }} px-4 py-3 rounded-xl transition duration-200 group relative">
                    <i class="fas fa-receipt w-5 text-lg transition-transform group-hover:scale-110 {{ request()->routeIs('admin.transactions.*') ? 'text-brand-400' : 'text-gray-500 group-hover:text-gray-300' }}"></i>
                ...
                <a href="{{ route('admin.users.index') }}" class="flex items-center space-x-3 {{ request()->routeIs('admin.users.*') ? 'bg-brand-500/10 text-brand-400' : 'text-gray-400 hover:bg-dark-700/50 hover:text-white' }} px-4 py-3 rounded-xl transition duration-200 group">
                    <i class="fas fa-users w-5 text-lg transition-transform group-hover:scale-110 {{ request()->routeIs('admin.users.*') ? 'text-brand-400' : 'text-gray-500 group-hover:text-gray-300' }}"></i>
                ...
                <a href="{{ route('admin.api-providers.index') }}" class="flex items-center space-x-3 {{ request()->routeIs('admin.api-providers.*') ? 'bg-brand-500/10 text-brand-400' : 'text-gray-400 hover:bg-dark-700/50 hover:text-white' }} px-4 py-3 rounded-xl transition duration-200 group">
            ...
            <div class="p-4 border-t border-dark-700 bg-dark-800 shrink-0" x-show="sidebarOpen">
                <div class="flex items-center gap-3 p-3 rounded-2xl bg-dark-900 border border-dark-700/50 shadow-inner group cursor-pointer hover:border-brand-500/30 transition">
                    <div class="relative">
                        <img src="https://ui-avatars.com/api/?name={{ urlencode(auth()->user()->name ?? 'Admin') }}&background=4F46E5&color=fff&bold=true" alt="User" class="h-10 w-10 rounded-xl shadow-md border border-dark-600">
                    ...
                    <form method="POST" action="{{ route('logout') }}" class="pl-1" @click.stop>
                        @csrf
            ...
            <header class="h-20 glass-panel sticky top-0 z-10 w-full flex items-center justify-between px-6 lg:px-10">
                <div class="flex items-center gap-5">
                    ...
                    <div class="hidden md:block">
                        <h1 class="text-xl font-extrabold text-white tracking-tight">@yield('header', 'Dashboard')</h1>
                        <div class="flex items-center text-xs font-medium text-gray-500 gap-2 mt-0.5">
                            <span class="hover:text-gray-300 cursor-pointer">{{ $global_site_name ?? 'PPOBKu' }} Admin</span> 
                            <i class="fas fa-chevron-right text-[8px] text-gray-600"></i> 
                            <span class="text-brand-400">@yield('header', 'Overview')</span>
                        </div>
                    </div>
                </div>
            ...
            <div class="flex-1 overflow-x-hidden overflow-y-auto p-4 sm:p-6 lg:p-8">
                ...
                @if(session('success'))
                    <div class="mb-6 flex items-center gap-4 bg-brand-500/10 border border-brand-500/20 text-brand-300 px-5 py-4 rounded-xl shadow-lg shadow-brand-500/5 animate-fade-in-down">
                        ...
                        <p class="text-sm font-semibold text-white tracking-wide">{{ session('success') }}</p>
                ...
                @if(session('error'))
                    <div class="mb-6 flex items-center gap-4 bg-red-500/10 border border-red-500/20 text-red-400 px-5 py-4 rounded-xl shadow-lg shadow-red-500/5 animate-fade-in-down">
                        ...
                        <p class="text-sm font-semibold text-white tracking-wide">{{ session('error') }}</p>
                        <button type="button" class="ml-auto text-gray-500 hover:text-white" onclick="this.parentElement.style.display='none'"><i class="fas fa-times"></i></button>
                    </div>
                @endif
                ...
                <footer class="mt-12 pt-6 pb-2 border-t border-dark-800 flex items-center justify-center text-xs font-medium text-gray-500">
                    <p>&copy; {{ date('Y') }} {{ $global_site_name ?? 'PPOBKu' }}. All rights reserved.</p>
                </footer>
            </div>
        </main>
    </div>
